...
public abstract class GruppoCarte {
	...
	public void sortBySeme(){
		Collections.sort(carte, this.semeComparator);
	}
	
	public void sortByValore(){
		Collections.sort(carte, this.valoreComparator);
	}
	
	public void sortByPrimiera(){
		Collections.sort(carte, this.primieraComparator);
	}
	...
	public int findPrimiera(){
		...
		for(Carta tempCarta: carte){
			switch(tempCarta.getSeme()){
				case COPPE:
					coppe  = Math.max(coppe, tempCarta.getPrimieraValue());  break;
				case DENARI:
					denari = Math.max(denari, tempCarta.getPrimieraValue()); break;
				case SPADE:
					spade  = Math.max(spade, tempCarta.getPrimieraValue());  break;
				case MAZZE:
					mazze  = Math.max(mazze, tempCarta.getPrimieraValue());  break;	
			};
		}
